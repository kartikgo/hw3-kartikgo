...
/**
 * This calculates overlaps of tokens, ngrams, part of speech tags(both tokens and ngrams),
 * lemma(bith tokens and ngrams) Finally a score which is a weighted sum of these attributes is
 * calculated.
 * 
 * @author Kartik Goyal
 * 
...
public class ScoreAnnotator extends JCasAnnotator_ImplBase {
  ...
  public void process(JCas jCas) throws AnalysisEngineProcessException {
    ...
    while (que.hasNext()) {
      ...
      while (ne.hasNext()) {
        NamedEntityMention nen = (NamedEntityMention) ne.next();
        if ((nen.getBegin() >= qbeg) && (nen.getEnd() <= qend)) {
          questionne.add(nen);
        }
      }
    }
    ...
    while (ans.hasNext()) {
      ...
      while (ne.hasNext()) {
        NamedEntityMention t = (NamedEntityMention) ne.next();
        if ((t.getBegin() >= abeg) && (t.getEnd() <= aend)) {
          answerne.add(t);
        }
      }
        ...
        for (NGram an : answerGram) {
          if (an.getElements().size() == qn.getElements().size()) {
            ...
            if (an.getElements().size() == 2) {
              ...
              if (aFirst.getCoveredText().equals(qFirst.getCoveredText())
                      && (aSecond.getCoveredText().equals(qSecond.getCoveredText()))) {
                gramOl += 1;
              }
              if (aFirst.getLemma().equals(qFirst.getLemma())
                      && (aSecond.getLemma().equals(qSecond.getLemma()))) {
                gramlemmaOl += 1;
              }
              if (aFirst.getPOS().equals(qFirst.getPOS())
                      && (aSecond.getPOS().equals(qSecond.getPOS()))) {
                gramPOSOl += 1;
              }
            }
            if (an.getElements().size() == 3) {
              ...
              if (aFirst.getCoveredText().equals(qFirst.getCoveredText())
                      && (aSecond.getCoveredText().equals(qSecond.getCoveredText()))
                      && (aThird.getCoveredText().equals(qThird.getCoveredText()))) {
                gramOl += 1;
              }
              if (aFirst.getLemma().equals(qFirst.getLemma())
                      && (aSecond.getLemma().equals(qSecond.getLemma()))
                      && (aThird.getLemma().equals(qThird.getLemma()))) {
                gramlemmaOl += 1;
              }
              if (aFirst.getPOS().equals(qFirst.getPOS())
                      && (aSecond.getPOS().equals(qSecond.getPOS()))
                      && (aThird.getPOS().equals(qThird.getPOS()))) {
                gramPOSOl += 1;
              }
            }
          }
